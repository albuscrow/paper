...
#ifdef NEW_MOVE
// 调整拟合出来的控制顶点
__global__ void move(TriangleD *triangleListD, float *triangleCtrlPointD, int *triangle_adjacent_tableD, float *triangleNormalCtrlPointD,
...
#define PN_NORMAL		// 如果定义了PN_NORMAL，就会看到异常情况
#ifdef PN_NORMAL
	int adjust_normal_idx[6] = { 9, 0,		0, 6,		6, 9 };
#else
	...
	for (int i = 0; i < 6; ++i)
	{
		...
		//已经作废float3 n_ctrlpoint_corner = make_float3(*(pn_x + corner_ctrlpoint_idx[i]), *(pn_y + corner_ctrlpoint_idx[i]), *(pn_z + corner_ctrlpoint_idx[i]));

#ifdef PN_NORMAL
		float3 n_ctrlpoint_corner = make_float3(pn_x[adjust_normal_idx[i]], pn_y[adjust_normal_idx[i]], pn_z[adjust_normal_idx[i]]);	// 能看到异常情况
#else
		float3 n_ctrlpoint_corner = triangleListD[triangleIdx].n_adj[adjust_normal_idx[i]];	// 论文中的算法
#endif
...
void calcTriangleCtrlPoint(bool adjust_silhouette, bool use_pn, AlgorithmType algo_type)
{
	...
	if (algo_type == CYM)
	{
		...
		if (adjust_silhouette)
		{
			...
			//cout << "move 开始" << endl;
#ifdef NEW_MOVE
			move<<<move_block_num, move_block_size>>>(triangleListD, triangleCtrlPointD, triangle_adjacent_tableD, triangleCtrlPointD + 3 * triangleNum * triangleCtrlPointNum_lower,
