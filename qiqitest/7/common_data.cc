...
float teapot_ctrl_point[3 * 4 * 4 * 36] = 
{
0, 0, 3.15, 
0, 0, 3.15, 
0, 0, 3.15, 
...
0, -1.3, 2.4, 
1.33333, 0, 2.4, 
1.33333, -0.746667, 2.4, 
0.746667, -1.33333, 2.4, 
0, -1.33333, 2.4, 
1.36667, 0, 2.4, 
1.33667, -0.765333, 2.4, 
0.765333, -1.33667, 2.4, 
0, -1.36667, 2.4, 
1.4, 0, 2.4, 
1.4, -0.784, 2.4, 
0.784, -1.4, 2.4, 
...
//#define ALGO_TEST
	...
	//m_nSamplePointCount = 3;		// Bézier 曲面片三角化时各方向的采样点数量为11
	m_nSamplePointCount = 30;		// Bézier 曲面片三角化时各方向的采样点数量为11
	//m_nSamplePointCount = 29;		// Bézier 曲面片三角化时各方向的采样点数量为11
#else
...
/* 载入 obj 模型 */
void CommonData::loadObj(const char *fileName, int u_seg, int v_seg)
{
	calcTimeLoad.start();
	...
	{
		teapot_ctrl_point[i * 3] += delta[X];
		teapot_ctrl_point[i * 3 + 1] += delta[Y];
		teapot_ctrl_point[i * 3 + 2] += delta[Z];
	}
	for (int i = 0; i < 3 * 16 * 36; ++i)
	{
		teapot_ctrl_point[i] /= maxXYZ;
	}
	//for (int i = 0; i < 16 * 36; ++i)
	//{
		//cout << "xyz = " << teapot_ctrl_point[i * 3] << " " <<
		//teapot_ctrl_point[i * 3 + 1] << " " <<
		//teapot_ctrl_point[i * 3 + 2] << endl;
	//}
...
void CommonData::calcFinalResult()
{
	...
	if (m_bGPU)
	{
		...
		if (m_eAlgorithm == AFFD)
		{
			...
			tessellateD_truth(adjust_silhouette_, m_bFirstLoad);
#ifdef TRUTH
			/*-------------------- 计算 ground truth 开始 ----------------------*/
