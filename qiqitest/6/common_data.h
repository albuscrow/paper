...
class CommonData
{
	...
	objdata::ObjData *objData, *objData_teapot;
	...
	std::vector<objdata::VertexCoord> vertexCoordList_teapot;	// 调整好之后的顶点几何坐标
	std::vector<objdata::NormalCoord> normalCoordList_teapot;	// 调整好之后的顶点几何坐标
...
public:
	...
	void loadObj(const char *fileName, const char *fileName2);
	/* 返回模型半径 */
	...
	unsigned int vertexCount_teapot() const {return vertexCoordList_teapot.size();}
	/* 返回模型面片数量 */
	...
	objdata::VertexCoord vertexParam_teapot(int idx) const {return vertexCoordList_teapot[idx];}
	objdata::NormalCoord normalParam_teapot(int idx) const {return normalCoordList_teapot[idx];}
	...
	std::vector<int> teapotFaceList;
	std::vector<int> belongs_to_origin;
	int u_seg, v_seg, u_seg_real, v_seg_real;
	int special_limit, before_special_per_surface, after_special_per_surface;
};
