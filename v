...
xnoremap <silent> ,sb :call VEnclose('', '', '\begin{sloppybar}', '\end{sloppybar}')
...
nnoremap <silent> <Plug>Tex_FastCommandInsert i=Tex_FastCommandInsert('no')
...
xnoremap <buffer> ;ll :call Tex_PartCompile()
...
xnoremap <buffer> <silent> ã `>a}`<i\mathcal{
xnoremap <buffer> <silent> â `>a}`<i\mathbf{
let &cpo=s:cpo_save
unlet s:cpo_save
setlocal keymap=
setlocal noarabic
...
56
normal! zo
let s:l = 121 - ((28 * winheight(0) + 23) / 47)
