...
extern GLuint triangleCtrlPointVBO;	// 结果控制顶点的缓冲区对象标识符
extern GLuint normalCtrlPointVBO;		// 结果控制顶点的缓冲区对象标识符
//#ifdef LINE
...
void View::genBuffers()
{
	...
	glGenBuffers(1, &triangleCtrlPointVBO);
	glGenBuffers(1, &normalCtrlPointVBO);
//#ifdef LINE
...
void View::delBuffers()
{
	...
	glBindBuffer(GL_ARRAY_BUFFER, triangleCtrlPointVBO);
	glDeleteBuffers(1, &triangleCtrlPointVBO);
	glBindBuffer(GL_ARRAY_BUFFER, normalCtrlPointVBO);
	glDeleteBuffers(1, &normalCtrlPointVBO);
//#ifdef LINE
...
void View::setBufferData()
{
	...
	int triangleNum = m_pCommonData->triangleCount();
	glBindBuffer(GL_ARRAY_BUFFER, triangleCtrlPointVBO);
	glBufferData(GL_ARRAY_BUFFER, sizeof(float) * 3 * triangleNum * 10, 0, GL_DYNAMIC_DRAW);
	cout << "@控制网格坐标VBO" << endl << "\t";
	viewMemD += sizeof(float) * 3 * triangleNum * 10;

	glBindBuffer(GL_ARRAY_BUFFER, normalCtrlPointVBO);
	glBufferData(GL_ARRAY_BUFFER, sizeof(float) * 3 * triangleNum * 10, 0, GL_DYNAMIC_DRAW);
	cout << "@法向控制网格坐标VBO" << endl << "\t";
	viewMemD += sizeof(float) * 3 * triangleNum * 10;

	const vector<float> &textureVector = m_pCommonData->getTextureVector();
	//for (int i = 0; i < triangleNum; ++i)
	//{
		//for (int j = 0; j < 10; ++j)
			//cout << "(" << textureVector[i * 20 + j * 2] << " "
				 //<< textureVector[i * 20 + j * 2 + 1] << ") ";
		//cout << endl;
	//}
	//cout << "v.size = " << textureVector.size() << endl;
	//cout << "f = " << triangleNum << endl;
	glBindBuffer(GL_ARRAY_BUFFER, texCoordVBO);
	glBufferData(GL_ARRAY_BUFFER, sizeof(float) * 2 * triangleNum * 10, &textureVector[0], GL_DYNAMIC_DRAW);
	cout << "@所有三角形(未细分)的二维纹理坐标VBO" << endl << "\t";
	viewMemD += sizeof(float) * 2 * triangleNum * 10;
...
void View::initializeGL()
{
	...
	string vertexShader, tcShader, teShader, gShader, fragmentShader;
#ifdef LINE
	readShaderSource("affd_line", vertexShader, tcShader, teShader, gShader, fragmentShader);
#else
#ifdef TRUTH
	readShaderSource("affd_truth", vertexShader, fragmentShader);
#else
	readShaderSource("affd", vertexShader, tcShader, teShader, fragmentShader);
#endif
#endif

#ifdef LINE
	bool success = installShaders(vertexShader.c_str(), tcShader.c_str(),
								  teShader.c_str(), gShader.c_str(), fragmentShader.c_str());
#else
	bool success = installShaders(vertexShader.c_str(), tcShader.c_str(),
								  teShader.c_str(), fragmentShader.c_str());
#endif
	if (!success)
	{
		...
		if ((m_nDisplayMode & DISPLAY_MODEL) != 0)			// 需要绘制模型
		{
			...
			else											// AFFD算法
			{
				if (m_pCommonData->isGPU())					// 使用GPU计算
				{
					...
					//glUniform1f(getUniLoc(prog, "TessLevelInner"), 8);
					//glUniform1f(getUniLoc(prog, "TessLevelOuter"), 8);
					...
					glBindBuffer(GL_ARRAY_BUFFER, triangleCtrlPointVBO);
					const GLuint index_mCtrlPoint = glGetAttribLocation(prog, "v_ctrlPoint");
					glVertexAttribPointer(index_mCtrlPoint, 3, GL_FLOAT, GL_FALSE, 0, 0);
					glEnableVertexAttribArray(index_mCtrlPoint);

					glBindBuffer(GL_ARRAY_BUFFER, normalCtrlPointVBO);
					const GLuint index_mCtrlPointN = glGetAttribLocation(prog, "v_ctrlPointN");
					glVertexAttribPointer(index_mCtrlPointN, 3, GL_FLOAT, GL_FALSE, 0, 0);
					glEnableVertexAttribArray(index_mCtrlPointN);
...
//#endif
					...
					//for (int i = 0; i < 1; ++i)
					{
						...
						MtlTex mtlTex = m_pCommonData->getMtl(i);
						//cout << "ka = " << mtlTex.m_fKa[0] << " " << mtlTex.m_fKa[1] << " " << mtlTex.m_fKa[2] << endl;
						//cout << "kd = " << mtlTex.m_fKd[0] << " " << mtlTex.m_fKd[1] << " " << mtlTex.m_fKd[2] << endl;
						//cout << "ks = " << mtlTex.m_fKs[0] << " " << mtlTex.m_fKs[1] << " " << mtlTex.m_fKs[2] << endl;
						//cout << endl;
						glUniform4f(ka_id_, mtlTex.m_fKa[0], mtlTex.m_fKa[1], mtlTex.m_fKa[2], 1.0);
						//if (i == 0)
						//glUniform4f(kd_id_, mtlTex.m_fKd[0], 0, 0, 1.0);
						//else if (i == 1)
						//glUniform4f(kd_id_, 0, mtlTex.m_fKd[1], 0, 1.0);
						//else if (i == 2)
						//glUniform4f(kd_id_, 0, 0, mtlTex.m_fKd[2], 1.0);
						//else
						...
						glDrawArrays(GL_PATCHES, 10 * ttt, 10 * m_pCommonData->mtlSize(i));
						//cout << "mtlSize = " << m_pCommonData->mtlSize(i);
...
//#endif
	...
	//while (!done) {
		////cout << "waiting" << endl;
		//glGetQueryObjectiv(queryID[0], GL_QUERY_RESULT_AVAILABLE, &done);
	//}
	////cout << "done! " << endl << endl;
	//glGetQueryObjectui64v(queryID[0], GL_QUERY_RESULT, &elapsed_time);
...
void View::keyPressEvent(QKeyEvent *event)
{
	...
	else if (event->key() == Qt::Key_R)
	{
		...
		qt_query.end();
		GLuint64 elapsed_time = qt_query.waitForResult();
		printf("Time Elapsed: %f ms\n", elapsed_time / 1000000.0);
		//glEndQuery(GL_TIME_ELAPSED);
		//GLuint64 elapsed_time;
		//int done = 0;
...
#ifndef LINE
// 四个shader，默认情况
int View::readShaderSource(const char *fileName, string &vertexShader, string &tcShader, string &teShader, string &fragmentShader)
{
	string vsFileName = string("../") + fileName + ".vs";
	string tcsFileName = string("../") + fileName + ".tcs";
	string tesFileName = string("../") + fileName + ".tes";
...
#else
// 五个shader，绘制线框
int View::readShaderSource(const char *fileName, string &vertexShader, string &tcShader, string &teShader, string &gShader, string &fragmentShader)
{
	string vsFileName = string("../") + fileName + ".vs";
	string tcsFileName = string("../") + fileName + ".tcs";
	string tesFileName = string("../") + fileName + ".tes";
	string gsFileName = string("../") + fileName + ".gs";
	string fsFileName = string("../") + fileName + ".fs";
...
#ifndef LINE
/* 安装shader，四个shader，默认情况 */
bool View::installShaders(const GLchar *vertexShader, const GLchar *tessCtrlShader, const GLchar *tessEvalShader, const GLchar *fragmentShader)
{
	...
	glShaderSource(tcs, 1, &tessCtrlShader, NULL);
	glShaderSource(tes, 1, &tessEvalShader, NULL);
	glShaderSource(fs, 1, &fragmentShader, NULL);

	GLint vert_status, tcs_status, tes_status, frag_status;
	...
	glCompileShader(tcs);
	glGetShaderiv(tcs, GL_COMPILE_STATUS, &tcs_status);
	if (tcs_status == GL_FALSE)
		printShaderInfoLog(tcs, "tess ctrl shader");

	glCompileShader(tes);
	glGetShaderiv(tes, GL_COMPILE_STATUS, &tes_status);
	if (tes_status == GL_FALSE)
		printShaderInfoLog(tes, "tess eval shader");
	...
	if ((vert_status == GL_FALSE) || (tcs_status == GL_FALSE) ||
		(tes_status == GL_FALSE) || (frag_status == GL_FALSE))
		return 0;
...
//#endif
	...
	int v[4] = {1, 1, 2, 6};
	//glUniform1iv(getUniLoc(prog, "factorial"), 10, v);
	float v2[4];
	for (int i = 0; i < 4; ++i)
		v2[i] = 1.0 / v[i];
	glUniform1fv(getUniLoc(prog, "factorial_1"), 4, v2);
...
#else
/* 安装shader，五个shader，绘制线框 */
bool View::installShaders(const GLchar *vertexShader, const GLchar *tessCtrlShader, const GLchar *tessEvalShader, const GLchar *gShader, const GLchar *fragmentShader)
{
	GLuint vs = glCreateShader(GL_VERTEX_SHADER);
	GLuint tcs = glCreateShader(GL_TESS_CONTROL_SHADER);
	GLuint tes = glCreateShader(GL_TESS_EVALUATION_SHADER);
	...
	GLint vert_status, tcs_status, tes_status, gs_status, frag_status;
	...
	glCompileShader(gs);
	glGetShaderiv(gs, GL_COMPILE_STATUS, &gs_status);
	if (gs_status == GL_FALSE)
		printShaderInfoLog(gs, "geometry shader");
	...
	if ((vert_status == GL_FALSE) || (tcs_status == GL_FALSE) ||
		(tes_status == GL_FALSE) || (gs_status == GL_FALSE) || (frag_status == GL_FALSE))
		return 0;
