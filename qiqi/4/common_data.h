 ...
 * 注意：
 *
 * 这个程序使用tess shader实现曲面片的绘制，
 * 不显式拿到结果顶点，所以没有TRUTH模式
 * 
 * 而LINE模式通过tess shader和geometry shader实现
 */
//#define LINE
...
class CommonData
{
	...
	std::vector<std::vector<int> > mtlFaceList;
	std::vector<float> textureVector;
public:
	std::vector<matrix_stack::Matrix4x4> cubemap_matrix_list_;
	const std::vector<float> &getTextureVector() const {return textureVector;}
	unsigned int triangleCount() const {return triangleList.size();}
	int morphStep, sourceIdx, targetIdx;
	CommonData();
	~CommonData();
	void setAlgorithmType();
	AlgorithmType getAlgorithmType() { return algorithm_type_; }
	int mtlSize(int i){return mtlFaceList[i].size();}
