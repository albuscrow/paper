...
GLuint triangleCtrlPointVBO = 0, normalCtrlPointVBO = 0;
float *triangleCtrlPointVBO_ptr, *normalCtrlPointVBO_ptr;
struct cudaGraphicsResource *triangleCtrlPointVBO_CUDA, *normalCtrlPointVBO_CUDA;

__global__ void copy(float *triangleCtrlPointVBO_ptr, float *normalCtrlPointVBO_ptr, float *triangleCtrlPointD, int activeThreadNumCopy, int ctrlPointNum, int triangleNum)
{
	int globalIdx = blockDim.x * blockIdx.x + threadIdx.x;
	if (globalIdx >= activeThreadNumCopy)
		return;

	int triangleIdx = globalIdx / ctrlPointNum;
	int localIdx = globalIdx % ctrlPointNum;

	triangleCtrlPointVBO_ptr[globalIdx * 3 + 0] = triangleCtrlPointD[triangleIdx * ctrlPointNum + localIdx];
	triangleCtrlPointVBO_ptr[globalIdx * 3 + 1] = triangleCtrlPointD[(triangleIdx + triangleNum) * ctrlPointNum + localIdx];
	triangleCtrlPointVBO_ptr[globalIdx * 3 + 2] = triangleCtrlPointD[(triangleIdx + triangleNum * 2) * ctrlPointNum + localIdx];

	normalCtrlPointVBO_ptr[globalIdx * 3 + 0] = triangleCtrlPointD[(triangleIdx + triangleNum * 3) * ctrlPointNum + localIdx];
	normalCtrlPointVBO_ptr[globalIdx * 3 + 1] = triangleCtrlPointD[(triangleIdx + triangleNum * 4) * ctrlPointNum + localIdx];
	normalCtrlPointVBO_ptr[globalIdx * 3 + 2] = triangleCtrlPointD[(triangleIdx + triangleNum * 5) * ctrlPointNum + localIdx];
}
...
//cudaThreadSynchronize();
	...
	//}
	//delete []test;
	// 以上是打印生成的三角Bezier曲面片控制顶点的代码

	// 上面的函数生成的控制顶点格式和Tess shader要求不符，重新拷贝
	cudaGraphicsMapResources(1, &triangleCtrlPointVBO_CUDA, 0);
	cudaGraphicsMapResources(1, &normalCtrlPointVBO_CUDA, 0);
	size_t size = sizeof(float) * triangleNum * triangleCtrlPointNum_lower * 3;
	cudaGraphicsResourceGetMappedPointer((void**)&triangleCtrlPointVBO_ptr, &size, triangleCtrlPointVBO_CUDA);
	cudaGraphicsResourceGetMappedPointer((void**)&normalCtrlPointVBO_ptr, &size, normalCtrlPointVBO_CUDA);

	int blockSizeCopy = 256, activeThreadNumCopy, blockNumCopy;
	activeThreadNumCopy = triangleCtrlPointNum_lower * triangleNum;
	blockNumCopy = ceil(static_cast<double>(activeThreadNumCopy) / blockSizeCopy);
	copy<<<blockNumCopy, blockSizeCopy>>>(triangleCtrlPointVBO_ptr, normalCtrlPointVBO_ptr, triangleCtrlPointD, activeThreadNumCopy, triangleCtrlPointNum_lower, triangleNum);

	cudaGraphicsUnmapResources(1, &triangleCtrlPointVBO_CUDA, 0);
	cudaGraphicsUnmapResources(1, &normalCtrlPointVBO_CUDA, 0);
}
...
void tessellateD_truth(bool firstLoad)
{
	...
	/* 体积误差 */
	double volume = 0.0, volume_truth = 0.0;
	//int segment = samplePointPerEdge - 1;
	for (int f = 0; f < triangleNum; ++f)
	{
		for (int i = 0; i < segmentPerEdge; ++i)
		{
			for (int j = 0; j <= i; ++j)
			{
				...
				// 精确结果
				v0x = result_truth[samplePointPerTriangle * 3 * f + triangleCoord(i, j) * 3 + 0];
				v0y = result_truth[samplePointPerTriangle * 3 * f + triangleCoord(i, j) * 3 + 1];
				v0z = result_truth[samplePointPerTriangle * 3 * f + triangleCoord(i, j) * 3 + 2];
				v1x = result_truth[samplePointPerTriangle * 3 * f + triangleCoord(i + 1, j) * 3 + 0];
				v1y = result_truth[samplePointPerTriangle * 3 * f + triangleCoord(i + 1, j) * 3 + 1];
				v1z = result_truth[samplePointPerTriangle * 3 * f + triangleCoord(i + 1, j) * 3 + 2];
				v2x = result_truth[samplePointPerTriangle * 3 * f + triangleCoord(i + 1, j + 1) * 3 + 0];
				v2y = result_truth[samplePointPerTriangle * 3 * f + triangleCoord(i + 1, j + 1) * 3 + 1];
				v2z = result_truth[samplePointPerTriangle * 3 * f + triangleCoord(i + 1, j + 1) * 3 + 2];
				volume_truth += (v0z + v1z + v2z) * ((v1x - v0x) * (v2y - v0y) - (v2x - v0x) * (v1y - v0y));
				if (i < segmentPerEdge - 1)
				{
					double v0x = result_truth[samplePointPerTriangle * 3 * f + triangleCoord(i + 1, j) * 3 + 0];
					double v0y = result_truth[samplePointPerTriangle * 3 * f + triangleCoord(i + 1, j) * 3 + 1];
					double v0z = result_truth[samplePointPerTriangle * 3 * f + triangleCoord(i + 1, j) * 3 + 2];
					double v1x = result_truth[samplePointPerTriangle * 3 * f + triangleCoord(i + 2, j + 1) * 3 + 0];
					double v1y = result_truth[samplePointPerTriangle * 3 * f + triangleCoord(i + 2, j + 1) * 3 + 1];
					double v1z = result_truth[samplePointPerTriangle * 3 * f + triangleCoord(i + 2, j + 1) * 3 + 2];
					double v2x = result_truth[samplePointPerTriangle * 3 * f + triangleCoord(i + 1, j + 1) * 3 + 0];
					double v2y = result_truth[samplePointPerTriangle * 3 * f + triangleCoord(i + 1, j + 1) * 3 + 1];
					double v2z = result_truth[samplePointPerTriangle * 3 * f + triangleCoord(i + 1, j + 1) * 3 + 2];
					volume_truth += (v0z + v1z + v2z) * ((v1x - v0x) * (v2y - v0y) - (v2x - v0x) * (v1y - v0y));
				}
			}
		}
	...
	volume /= 6;
	volume_truth /= 6;
	cout << "近似体积 = " << volume << ", 真实体积 = " << volume_truth << endl;
	cout << "体积误差 = " << volume - volume_truth << ", 误差率 = " << fabs(volume - volume_truth) / volume_truth << endl;
...
void regGLBuffer()
{
	if (registered)
	{
		...
		cudaGraphicsUnregisterResource(triangleCtrlPointVBO_CUDA);
		cudaGraphicsUnregisterResource(normalCtrlPointVBO_CUDA);
#ifdef LINE
	...
	cudaGraphicsGLRegisterBuffer(&triangleCtrlPointVBO_CUDA, triangleCtrlPointVBO, cudaGraphicsMapFlagsWriteDiscard);
	cudaGraphicsGLRegisterBuffer(&normalCtrlPointVBO_CUDA, normalCtrlPointVBO, cudaGraphicsMapFlagsWriteDiscard);
