...
#define NEW_TRIANGULATE
#ifdef NEW_TRIANGULATE
void CommonData::triangulatePolygon()
{
	//vector<vector<int> > mtlFaceList(objData->mtlTexList.size() + 1);
	vector <vector<Triangle> > classifyList;

	mtlFaceList.clear();
	textureVector.clear();
	vector<int> a;
	vector<Triangle> b;
	for (vector<vector<int> >::size_type i = 0; i < objData->mtlTexList.size() + 1; ++i)
	{
		mtlFaceList.push_back(a);
		classifyList.push_back(b);
	}
				...
				for (unsigned int p = 0; p < trimmedPolygonList[i][j][k].size(); ++p)
				{
					...
					for (int q = 1; q < size - 1; ++q)
					{
						...
						if (useTex)
						{
							...
							triangleList.push_back(t);
							classifyList[mtlIdx].push_back(t);
	...
	triangleList.clear();
	int triangleCtrlPointNum = 10;
	cout << "clist.size = " << classifyList.size() << endl;
	for (vector<vector<Triangle> >::size_type i = 0; i < classifyList.size(); ++i)
	{
		cout << "ss = " << classifyList[i].size() << endl;
		for (vector<Triangle>::size_type j = 0; j < classifyList[i].size(); ++j)
		{
			triangleList.push_back(classifyList[i][j]);
			float u0 = classifyList[i][j].vt[0].u();
			float v0 = classifyList[i][j].vt[0].v();
			float u1 = classifyList[i][j].vt[1].u();
			float v1 = classifyList[i][j].vt[1].v();
			float u2 = classifyList[i][j].vt[2].u();
			float v2 = classifyList[i][j].vt[2].v();
			textureVector.push_back(u0);
			textureVector.push_back(v0);
			textureVector.push_back(u1);
			textureVector.push_back(v1);
			textureVector.push_back(u2);
			textureVector.push_back(v2);
			for (int k = 0; k < (triangleCtrlPointNum - 3) * 2; ++k)
			{
