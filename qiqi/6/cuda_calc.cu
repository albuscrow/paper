...
__global__ void my_to_truth(int f, int q, int point_per_real_face_u, int point_per_real_face_v,
		float *myV, float *realV, float *texCoord, int *my_to_truth_tableD, int *belongs_to_originD)
{
...
//#define OLD
	...
	for (int i = 0; i < point_per_real_face_u * point_per_real_face_v; ++i)
	{
...
#else
	float u = texCoord[globalIdx * 2];
	float v = texCoord[globalIdx * 2 + 1];
...
void tessellateD_truth(bool adjust_silhouette, bool firstLoad, vector<int> &teapotFaceList, vector<int> &belongs_to_origin, int u_seg, int v_seg)
{
	...
	if (firstLoad)
	{
		...
		my_to_truth<<<block_num, block_size>>>(triangleNum, samplePointPerTriangle,
				(u_seg + 1), (v_seg + 1), vertexPtrVBO, vertexPtrVBO_truth, texCoordPtrVBO,
