...
void View::setBufferData()
{
	...
	//cout << "@二维纹理坐标VBO" << endl << "\t";
	viewMemD += sizeof(float) * tessPointCount * 3;
	printMemD(__FILE__, __FUNCTION__, __LINE__, sizeof(float) * tessPointCount * 3, "@二维纹理坐标VBO");
...
void View::paintGL()										// 完成opengl绘制的函数
{
			...
			else											// AFFD算法
			{
				if (m_pCommonData->isGPU())					// 使用GPU计算
				{
					...
					else
					{
						...
						glVertexAttribPointer(index_mTexCoord, 3, GL_FLOAT, GL_FALSE, 0, 0);
	...
	//#endif
						...
						for (int i = 0; i < m_pCommonData->objMtlTexListSize() + 1; ++i)
						{
							...
							//glDrawElements(GL_TRIANGLES, 29 * 29 * 3, GL_UNSIGNED_INT, 0);
